...
function controlClick() {
    $tablero.onclick = function (e) {
        let cuadroMarcado = e.target;
        if (cuadroMarcado.classList.contains("col")) {
            manejarRonda (cuadroMarcado)
        }
}
};
...
function manejarRonda (cuadroMarcado) {
    ...
    if (ronda%2 === 0 ) {
        if (document.querySelectorAll (".hiden").length !== 0) {
            function vacia () {}
        }

        if ((seleccionado[seleccionado.length-1] !== seleccionado[seleccionado.length-2]) & 
            (seleccionado[seleccionado.length-1].className === seleccionado[seleccionado.length-2].className)) {
            seleccionado[seleccionado.length-1].className = "hiden";
            seleccionado[seleccionado.length-2].className = "hiden";
        }  
    ...
    if ($cuadros.length === 0) {
        document.querySelector ("h1").innerText = "Ganaste!!!"
        alert ("Felicitaciones ganaste el jeugo!");
    }
}}};
...
function configurarCuadros () {
    shuffle(base);

base.forEach (function (color, index) {
